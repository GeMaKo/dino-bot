...
def last_seen_sum_patrol_point_evaluator(
    ...
    if {
        enemy.position for enemy in game_state.visible_bots
    } in game_state.visibility_map[move]:
        enemy_penalty = 10000  # Large penalty for enemy in range
    if set(game_state.last_n_ticks_bot_positions).intersection(
        game_state.visibility_map[move]
    ):
    ...
    for seen_tile in game_state.visibility_map[move]:
